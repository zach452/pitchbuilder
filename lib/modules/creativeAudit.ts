...
export async function generateCreativeAudit(
  ...
  const files = listFilesForProject(projectId).filter((f) => f.source_type === "creative_scorecard");
    ...
    try {
      ...
      for (const sheet of result.sheets) {
        for (const row of sheet.rows) {
          ...
          const grade = creativeGrade(rawScore);
          scores.push({
            asset_name: name,
            score: rawScore,
            grade,
  ...
  createEvidence({
    ...
    extracted_text: `Creative scorecard parsed: ${scores.length} assets. Grade distribution: ${JSON.stringify(
      gradeDistribution
    )}`,
  ...
  const evidence = getEvidenceForProject(projectId).filter(
    (e) => e.source_type === "creative_scorecard"
  );
  const system = loadPrompt("creative_audit");
  const user = `Parsed creative scores: ${JSON.stringify(
    scores
  )}\n\nGrade distribution: ${JSON.stringify(
    gradeDistribution
  ...
  saveArtifact(projectId, "creative_findings", result);
  return result;
}
